...
HELP_MESSAGE = '''
Данный телеграм-бот был создан для упрощенного получения виртуальных номеров, которые можно регистрировать в определенных сервисах, например, в приложениях по доставке продуктов. Для этого он использует API сайта sms-activate.ru\n\nЧтобы сайт понял, кто общается с ним через бота-помощника, и начал делиться виртуальными номерами, укажи свой API-ключ, который ты найдешь в личном кабинете после регистрации на сайте\n\n
/send_api_key - отправить свой API-ключ\n
'''
